...
//DAO 클래스(Repository 클래스) : 저장매개체(DBMS)의 행에 대한 삽입, 삭제, 변경, 삭제, 검색기능을 
//제공하기 위한 클래스
//=> DAO 클래스의 메소드는 DBMS 서버에 접속하여 하나의 SQL명령을 전달하여 실행하고 실행결과를 
//Java 객체(값)로 반환되도록 작성 - JDBC
// => DAO 클래스가 교체되도 의존관계로 설정된 Service 클래스에 영향을 최소화 하기 위해 인터페이스를 
//상속받아 작성하는 것을 권장
...
//=> SpringMVC 프레임워크에서 스프링 컨테이너를 초기화 처리하기 위한 Spring Bean Configuration File 에서
//Bean 엘리먼트로 클래스를 Spring Bean 으로 등록 - root-context.xml 또는 servlet-context.xml
