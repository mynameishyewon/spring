...
public class RedirectController {
	...
	public String forward(Model model) {
		...
		// => Front controller(DispatchServlet 객체)는 반환받은 뷰이름을
		// InternalResourceViewResolver
		// 객체에게 전달하여 JSP 문서의 경로를 제공받아 포워드 이동하여 JSP 문서로 응답 처리
		// 포워드(Forward) : 클라이언트 요청에 의해 실행된 웹프로그램(Front Controller)에서 응답 처리할
		// 웹프로그램(View - JSP)으로 스레드를 이동하여 명령을 실행
	...
	public String redirect() {
		// 요청 처리 메소드의 반환값(뷰이름)에 redirect 접두사를 사용하면 클라이언트에게
		//
	...
	*/
	...
	//객체를 제공받아 사용 가능
	//=> RedirectAttrivutes 객체 : 리다이렉트 이동되는 페이지의 요청 처리 메소드와 뷰에서
