...
//메일 전송 기능을 제공하기 위한 클래스
//=> 메일 서버(Mail Server) : 메일을 송수신하는 서비스를 제공하는 컴퓨터
//=> STMP 서비스(Simple Message Transfer Protocol - 25)로 메일을 보내고 POP3(Post Office Protocaol3 - 110) 서비스나 IMAP 서비스는 메일을 받아 사용자에게 전달하는 서비스
...
public class EmailSendBean {
	...
	//=> 메일을 받는 사람의 이메일 주소, 제목, 내용을 매개변수로 전달받아 사용
	...
	public String sendEmail(String email, String subject, String content) throws Exception {
		...
		//MimeMessage.setText(String content) : MimeMessage 객체의 메일내용(일반문서)을 변경하는 메소드
		//=> 메일 내용을 일반 텍스트 문서 형태로 저장하여 전달
		//message.setText(content); 얘는 html 컨텐츠를 이해 못함
		
		//MimeMessage.setContent(Object o,String type) : MimeMessage 객체의 메일 내용을 변경하
		//=> type 매개변수에 메일로 전달할 문서의 형식(MimeType)을 전달하여 저장
		//=> 메일 내용을 HTML 문서 형태로 저장하여 전달
		message.setContent(content, "text/html; charset=utf-8"); 
		
		//MimeMessage.setRecipient(Recipient type, Address address) : MimeMessage 객체의
		//메일을 받는 사람의 이멩리 주소 관련 정보를 변경하는 메소드
		//=> type RecipientType : 메일 수신 사용자를 구분하기 위한 상수 전달
		...
		//JavaMailSender.send(MimeMessage message) : 매개변수로 전달된 MimeMessage 객체의 정보로
