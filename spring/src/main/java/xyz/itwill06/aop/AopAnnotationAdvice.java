...
public class AopAnnotationAdvice {
	...
	// @Before : 타겟메소드의 명령(핵심관심코드)이 실행되기 전에 삽입되어 실행될 횡단관심코드
	// 제공하기 위한 어노테이션
	...
	// AfterReturning : 타겟메소드의 명령(핵심관심코드)이 실행된 후에 예외 발생과 상관없이 무조건
	// 삽입되어 실행될 횡단관심코드를 제공하기 위한 어노테이션
	...
	//@AfterThrowing : 타겟메소드의 명령(핵심관심코드)이 실행되면서 예외가 발생되면 삽입
	//throwing 속성 : 타겟메소드에서 예외를 제공받아 저장할 매개변수의 이름을 속성값으로
	...
	public Object aroundLog(ProceedingJoinPoint joinPoint) throws Throwable {
		log.info("[around]핵심관심코드가 실행 전에 삽입되어 실행될 횡단관심코드");
		
		Object result=joinPoint.proceed();
		log.info("[around]핵심관심코드가 실행 후에 삽입되어 실행될 횡단관심코드");
		return result;
	}
}
